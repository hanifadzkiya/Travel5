...
const insertTransaction = async (username, jenisTransaksi, detailTransaksi) => {
  ...
  require("fs").writeFileSync(`public/images/${transactionId}.png`, qrcode.replace(/^data:image\/png;base64,/, ""), 'base64');
  const urlImage = `http://${commonUtil.getPublicIp()}/images/${transactionId}.png`;
  emailUtil.sendEmail(result.email, "Your Package is Ready", `Please pay your bill in <a href="${urlPayment}"> here </a> Qr code : <img src="${urlImage}" height="400" width="400"/>`);
  return result;
};
